  ...
  return (
    <div>
      ...
      <div style={{background:'#fff',padding:'60px 48px 48px',borderBottom:'.5px solid rgba(0,0,0,.08)',textAlign:'center'}}>
        ...
        <p style={{fontSize:17,color:'#6e6e73',maxWidth:540,margin:'0 auto',lineHeight:1.47,fontWeight:400}}>A concise overview of Smart Saathi — the problem it addresses, the solution it delivers, and the technology that powers it.</p>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:52,padding:'60px 48px',background:'#f5f5f7',alignItems:'flex-start',maxWidth:1200,margin:'0 auto'}}>
        <div>
          ...
          <div style={{background:'#fff',borderRadius:18,padding:24,marginBottom:14,boxShadow:'0 2px 8px rgba(0,0,0,.04),0 12px 28px rgba(0,0,0,.06)'}}>
            ...
            <p style={{fontSize:17,fontWeight:700,color:'#1d1d1f',marginBottom:10,letterSpacing:'-.02em'}}>Smart Saathi — An AI-powered elderly care companion</p>
            <p style={{fontSize:13,color:'#6e6e73',lineHeight:1.6,marginBottom:10}}>The elderly population worldwide faces increasing challenges as life expectancy rises and more seniors live independently. Age-related issues such as memory decline, physical frailty, chronic illnesses, and isolation heighten the risk of health incidents. Smart Saathi was developed to address these concerns by combining artificial intelligence, speech recognition, and healthcare IoT into a user-friendly platform.</p>
            <p style={{fontSize:13,color:'#6e6e73',lineHeight:1.6,marginBottom:12}}>Unlike conventional health applications that rely on complex touch interfaces and small text — often inaccessible to seniors — Smart Saathi emphasises simplicity through natural voice interaction. By capturing voice commands and health data, the system generates a real-time picture of an individual's well-being, enabling family members to monitor medication adherence, respond to SOS alerts, and plan medical appointments.</p>
          ...
          <div style={{background:'#fff',borderRadius:18,padding:24,marginBottom:14,boxShadow:'0 2px 8px rgba(0,0,0,.04),0 12px 28px rgba(0,0,0,.06)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              {[
                {c:'#ff3b30',l:'❌ Generic voice assistants',d:'No healthcare-specific features, no extra monitoring dashboards, no emergency SOS'},
                {c:'#ff3b30',l:'❌ Medical alert necklaces',d:'Hardware-dependent, stigmatising, often resisted by seniors'},
                {c:'#1975d2',l:'✓ Smart Saathi',d:'Voice-first, no proprietary hardware, seamless family dashboard integration'},
                {c:'#1975d2',l:'✓ Holistic ecosystem',d:'Medication, SOS, location, and family connectivity in one unified platform'},
              ].map(({c,l,d})=>(
          ...
          <div style={{background:'#fff',borderRadius:18,padding:24,boxShadow:'0 2px 8px rgba(0,0,0,.04),0 12px 28px rgba(0,0,0,.06)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
              {[['📱','Flutter','Cross-platform mobile (Android/iOS)'],['⚛️','React.js','Real-time family dashboard'],['🟢','Node.js','Event-driven backend & APIs'],['🍃','Supabase','Database, auth & realtime'],['🎙️','Google STT/TTS','Voice AI — Speech & synthesis'],['🧠','NLP Intent','Command recognition & classification']].map(([icon,name,desc])=>(
