...
class _SeniorHomeScreenState extends ConsumerState<SeniorHomeScreen> {
  ...
  Future<void> _startVoiceCommand() async {
    setState(() { _isListening = true; _lastCommand = ''; _lastResponse = ''; });
    try {
      ...
      final command = await completer.future.timeout(const Duration(seconds: 6), onTimeout: () {
        voiceService.stopListening();
        return '';
      });
      voiceService.stopListening();
      setState(() { _isListening = false; _lastCommand = command; });
      ...
      switch (intent) {
        ...
        case 'SOS':
          ...
          await ref.read(supabaseServiceProvider).triggerSOS(
            pos?.latitude ?? 28.61, pos?.longitude ?? 77.20,
          );
          break;
        case 'SYMPTOM_TRIAGE':
          await ref.read(supabaseServiceProvider).addHealthLog(symptoms: command);
          break;
        case 'LOG_BP':
          final bp = double.tryParse(result['entity'] ?? '');
          if (bp != null) await ref.read(supabaseServiceProvider).addHealthLog(bp: bp);
          break;
        case 'LOG_SUGAR':
          final sugar = double.tryParse(result['entity'] ?? '');
          if (sugar != null) await ref.read(supabaseServiceProvider).addHealthLog(sugar: sugar);
          break;
        case 'LOG_TEMPERATURE':
          final temp = double.tryParse(result['entity'] ?? '');
          if (temp != null) await ref.read(supabaseServiceProvider).addHealthLog(temperature: temp);
          break;
      }

      setState(() => _lastResponse = response);
      try { await ref.read(voiceServiceProvider).speakHindi(response); } catch (_) {}
    } catch (e) {
      setState(() { _isListening = false; _lastResponse = 'Error: $e'; });
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              profileAsync.when(
                data: (profile) {
                  ...
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(greeting, style: TextStyle(fontSize: 18, color: Colors.grey.shade600)),
                      const SizedBox(height: 4),
                      Text(name, style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
                    ],
                  );
                },
                loading: () => const SizedBox(height: 56),
                error: (_, __) => const Text('Hello!', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
              ),
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: (_isListening ? const Color(0xFFFF5722) : const Color(0xFF2196F3)).withAlpha(80),
                          blurRadius: 24,
              ...
              if (_lastResponse.isNotEmpty) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: const Color(0xFF2196F3).withAlpha(30)),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      Text(_lastResponse, style: const TextStyle(fontSize: 18, color: Color(0xFF1A1A2E))),
                    ],
                  ),
                ),
              ],
              ...
              const Text('Upcoming Medications', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
                ...
                data: (meds) {
                  ...
                  if (pending.isEmpty) {
                    return Container(
                      ...
                      decoration: BoxDecoration(color: const Color(0xFF4CAF50).withAlpha(15), borderRadius: BorderRadius.circular(16)),
                      child: const Row(
                        children: [
                          Icon(Icons.check_circle, color: Color(0xFF4CAF50), size: 32),
                          SizedBox(width: 12),
                          Text('All medications taken! ✅', style: TextStyle(fontSize: 18, color: Color(0xFF4CAF50))),
                        ],
                      ),
                    );
                  }
                  return Column(
                    children: pending.take(3).map((m) => _buildMedCard(m.name, m.dose, m.time, m.id)).toList(),
                  );
                },
              ...
              // ── HYDRATION TRACKER ───────────────
              const Text('Hydration Today', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
              const SizedBox(height: 12),
              hydrationAsync.when(
                data: (glasses) => _buildHydrationCard(glasses),
                loading: () => const Center(child: CircularProgressIndicator()),
                error: (_, __) => _buildHydrationCard(0),
              ),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.medication, color: Color(0xFFFF5722), size: 28),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('$dose • $time', style: TextStyle(fontSize: 14, color: Colors.grey.shade500)),
              ],
            ),
          ),
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('Taken', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
            ),
          ),
        ],
      ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Water Glasses', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
                Text(glasses >= goal ? 'Goal reached! 🎉' : '${goal - glasses} more to go',
