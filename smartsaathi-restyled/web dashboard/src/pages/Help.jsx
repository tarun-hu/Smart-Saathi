...
const Features = () => {
  ...
  const seniorFeats = [
    {n:1,title:'🆘 One-tap SOS',desc:'A single button press triggers an instant alert to all caregivers with live GPS location and timestamp. Voice-activated distress detection recognises phrases like "Help, I have fallen" and responds automatically in under 2 seconds.',tag:'Real-time · <2s delivery'},
    ...
    {n:3,title:'💊 Medication Reminders',desc:'TTS-powered voice reminders notify users at the exact scheduled time without requiring menu navigation. Missed doses are logged and caregivers are instantly alerted. Particularly valuable for managing diabetes, hypertension, or cardiovascular conditions.',tag:'Missed dose alerts'},
    {n:4,title:'🧩 Cognitive Exercises',desc:'Daily brain exercises — memory recall, pattern recognition, verbal fluency — with scoring and trend tracking. Scores are surfaced in the caregiver dashboard as brain health metrics over time.',tag:'Brain health · Daily'},
  ];
  const careFeats = [
    ...
    {n:6,title:'📍 Live Location Tracking',desc:'Caregivers view the senior\'s location on a live map. Geofence alerts trigger notifications when the senior leaves a pre-defined safe zone. Location is shared automatically during SOS events.',tag:'GPS · Geofencing'},
    ...
    {n:8,title:'⚠️ Anomaly Detection',desc:'AI flags deviations from the senior\'s established routine — unusually sedentary periods, missed check-ins, irregular vitals — as anomalies surfaced prominently in the dashboard for caregiver review.',tag:'AI-powered · Routine-based'},
  ];
  ...
  return (
    <div>
      ...
      <div style={{background:'#fff',padding:'60px 48px 48px',borderBottom:'.5px solid rgba(0,0,0,.08)',textAlign:'center'}}>
        ...
        <h1 style={{fontSize:40,fontWeight:800,letterSpacing:'-.04em',color:'#1d1d1f',marginBottom:10,lineHeight:1.06}}>Everything SmartSaathi can do</h1>
        <p style={{fontSize:17,color:'#6e6e73',maxWidth:580,margin:'0 auto',lineHeight:1.47,fontWeight:400}}>Eight core capabilities designed around the real needs of elderly users and their caregivers, distilled from research and the SmartSaathi project objectives.</p>
      </div>
      ...
      <section style={{padding:'72px 48px',background:'#f5f5f7'}}>
        <div style={{display:'grid',gridTemplateColumns:'auto 1fr',gap:48,alignItems:'center',maxWidth:1100,margin:'0 auto'}}>
          ...
          <div>
            <p style={{fontSize:12,fontWeight:600,color:'#1975d2',textTransform:'uppercase',letterSpacing:'.1em',marginBottom:20}}>Caregiver-facing features</p>
            <div style={{display:'flex',flexDirection:'column',gap:22}}>
              {careFeats.map(f=>(
                <div key={f.n} style={{display:'flex',gap:14}}>
                  <NumBox n={f.n}/>
